...
def init_database():
    ...
    with db_connection() as conn:
        ...
        """)
        # Локальная демо-сборка: задаём фиксированный пароль для owner.
        # При каждом запуске хэш обновляется, поэтому пароль работает и для
        # уже существующей локальной базы данных.
        owner_password = "BK8pilWygbmbU1"
        conn.execute(
            "UPDATE users SET password_hash = ?, is_admin = 1 WHERE username = ?",
            (generate_password_hash(owner_password), "owner"),
        )
